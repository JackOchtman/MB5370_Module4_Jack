...
This script analyses shark drumline catch data from qfish.fisheries.qld.gov.au, separated into sharks \<2m in size and ≥2m in size.
...
```{r column setup echo = T, results = 'hide'}
shark_size <- 
  shark |>
...
```{r Data correction echo = T, results = 'hide'}
...
```{r plot small shark echo = T, results = 'show'}
...
```{r plot large shark echo = T, results = 'show'}
...
```{r final plot echo = T, results = 'show'}
facet_plot <- ggplot() +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 90, hjust = 1)) + # Removes 
  labs(
...
print(facet_plot) # Opens plot in script
```
