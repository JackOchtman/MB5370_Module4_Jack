...
```{r load file echo = T, results = 'show'}
facet_plot <- ggplot() +
  geom_smooth(data = shark_size, aes(x = year, y = small, color = "<2m"), method = "loess", se = FALSE) +
  geom_smooth(data = shark_size, aes(x = year, y = large, color = "≥2m"), method = "loess", se = FALSE) +
  facet_wrap(~ area) +
  scale_color_manual(values = c("<2m" = "#E69F00", "≥2m" = "#56B4E9")) +  # Manually set colors for Small and Large
  theme_minimal() +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
