...
<<<<<<< HEAD
Answer: When using variables that have a small set range of values and is continuous.
=======
Answer: When using variables that have a small set range of values and are continuous.
...
Troubleshooting error from workbook, the + should be on the top line
...
Fixed, "+" should never be on a new file.
...
Use facet wrap to split the output into sub plot.
...
nrow and ncol control the number of rows (nrow) and columns (ncol).  Other options for individual panels include "scales", "shrink", "drop", "as.table" "axes" and "axes labels".
...
```{r}
# Same as above but using geom_line
ggplot(data = mpg) + 
  geom_line(mapping = aes(x = displ, y = hwy))
```

Answer: Using geom_line doesn't work because it assumes continuous data.
...
It stops ggplot from computing the standard error.
...
Line chart output, this was already used earlier in the section
...
```
This produces a single massive box for everything.
...
This returns a seperate boxplot for different cylinder counts, but isn't particularly useful for this data.
...
```
Returns a histogram of highway efficiency, useful to check data distribution for analysis.
...
Very ugly area chart for displacement and highway efficiency.
...
They look the same, the code illustrates two different methods for selecting geometry in ggplot.
...
Answer:  Because we now have multiple different options that could be in the place of the y axis.
...
ggplot automatically uses the more accessible colour palatte for certain data types.
